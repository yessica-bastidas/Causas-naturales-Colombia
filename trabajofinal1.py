...
# cargar la base
df = pd.read_csv('datos_final.csv') # base 
...
if selected == "Personas":
    ...
    #cual es el mes en el que ocurren más inundaciones?
    ...
    # crear base ##NO SE SI HACERLO CON COUNT O SUM###########################################################################
...
if selected == "Viviendas":
    ...
    if st.checkbox('Obtener datos por vivienda, evento y mes', False):
        ...
        # Código para generar el DataFrame
        df0 = df.groupby(['tipo_de_evento','fecha','departamento'])[['viviendas_destruidas']].sum().reset_index().rename(columns={'tipo_de_evento':'evento','fecha':'mes'}).sort_values('viviendas_destruidas', ascending = False)
        ...
        fig = go.Figure(data=[go.Table(
            ...
            fill_color='lightblue',
            line_color='darkslategray'),
            cells=dict(values=[df0.evento,df0.mes,df0.departamento, df0.viviendas_destruidas],fill_color='white',line_color='lightgrey'))
           ])
    ...
    if st.checkbox('Obtener datos por vivienda afectada, evento y mes', False):
        ...
        # Código para generar el DataFrame
        df0 = df.groupby(['tipo_de_evento','fecha','departamento'])[['viviendas_averiadas']].sum().reset_index().rename(columns={'tipo_de_evento':'evento','fecha':'mes'}).sort_values('viviendas_averiadas', ascending = False)
        df0['mes'] = df0['mes'].dt.month
        # Código para convertir el DataFrame en una tabla plotly resumen
        fig = go.Figure(data=[go.Table(
            header=dict(values=list(df0.columns),
            ...
            line_color='darkslategray'),
            cells=dict(values=[df0.evento,df0.mes,df0.departamento, df0.viviendas_averiadas],fill_color='white',line_color='lightgrey'))
           ])
        fig.update_layout(width=600, height=400)
    ...
    df0=df.groupby(['departamento'])['viviendas_averiadas']
#------------------------------------------------------------    
